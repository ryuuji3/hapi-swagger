...
lab.experiment("group", () => {
    ...
    lab.test("test groups tagging of paths", async () => {
        ...
        expect(result.paths["/actors"].get.tags[0]).to.equal("actors");
        expect(result.paths["/movies"].get.tags[0]).to.equal("movies");
        expect(result.paths["/movies/movie"].get.tags[0]).to.equal("movies");
        expect(result.paths["/movies/movie/actor"].get.tags[0]).to.equal("movies");
    });
