...
export default class Properties {
    ...
    public simpleTypePropertyMap: object;
    public complexTypePropertyMap: object;
    ...
    constructor(
        ...
        public altDefinitionCollection?,
     ...
     * @return {Object}
     */
    public parseProperty(name, joiObj, parent, parameterType, useDefinitions, isAlt) {
        let property: {
            type?: string;
            format?: any;
            enum?: any;
        ...
        // add enum
        const describe = (joiObj as AnySchema).describe();
        ...
        if (allowDropdown && Array.isArray(describe.valids) && describe.valids.length) {
            ...
            const enums = describe.valids.filter(item => {
        ...
        if (property.type === "object") {
            if (Utilities.hasJoiChildren(joiObj)) {
                ...
                if (useDefinitions === true) {
                    const refName = this.definitions.append(
                        name,
                        property,
                        this.getDefinitionCollection(isAlt),
                        this.settings
                    );
                    property = {
     ...
     * @return {Object}
     */
    public parsePropertyMetadata(property, name, parent, joiObj) {
        ...
        if (this.settings.xProperties === true) {
            this.convertRules(property, describe.rules, ["unit"], "x-format");
     ...
     * @return {Object}
     */
    public parseString(property, joiObj) {
        const describe = joiObj.describe();
        ...
        // add extended properties not part of openAPI spec
        if (this.settings.xProperties === true) {
            this.convertRules(property, describe.rules, ["insensitive", "length"], "x-constraint");
            ...
            this.convertRules(property, describe.rules, ["lowercase", "uppercase", "trim"], "x-convert");
        }
     ...
     * @return {Object}
     */
    public parseNumber(property, joiObj) {
        const describe = joiObj.describe();
        ...
        if (this.settings.xProperties === true) {
            this.convertRules(
                property,
                describe.rules,
                ["greater", "less", "precision", "multiple", "positive", "negative"],
                "x-constraint"
            );
        }
     ...
     * @return {Object}
     */
    public parseDate(property, joiObj) {
     ...
     * @return {Object}
     */
    public parseObject(property, joiObj, name, parameterType, useDefinitions, isAlt) {
        property.properties = {};
     ...
     * @return {Object}
     */
    public parseArray(property, joiObj, name, parameterType, useDefinitions, isAlt) {
        const describe = joiObj.describe();
        ...
        // add extended properties not part of openAPI spec
        if (this.settings.xProperties === true) {
            this.convertRules(property, describe.rules, ["length", "unique"], "x-constraint");
        ...
        const arrayItemTypes = joiObj._inner.items; // joiObj._inner.inclusions;
        ...
        if (arrayItem) {
            ...
            if (this.simpleTypePropertyMap[arrayItem._type.toLowerCase()]) {
                ...
                for (const key in arrayItemProperty) {
                    if (arrayItemProperty.hasOwnProperty(key)) {
     ...
     * @return {Object}
     */
    public parseAlternatives(property, joiObj, name, parameterType, useDefinitions) {
        // convert .try() alternatives structures
        if (Hoek.reach(joiObj, "_inner.matches.0.schema")) {
            // add first into definitionCollection
            const child = joiObj._inner.matches[0].schema;
            ...
            if (this.settings.xProperties === true) {
                const altArray = joiObj._inner.matches
                    .map(obj => {
                        const altName = Utilities.getJoiLabel(obj.schema) || name;
                        // name, joiObj, parent, parameterType, useDefinitions, isAlt
        ...
        // convert .when() alternatives structures
            ...
            // add first into definitionCollection
            const child = joiObj._inner.matches[0].then;
            const childName = Utilities.getJoiLabel(child) || name;
            // name, joiObj, parent, parameterType, useDefinitions, isAlt
            property = this.parseProperty(childName, child, property, parameterType, useDefinitions, false);
            ...
            if (property && this.settings.xProperties === true) {
                const altArray = joiObj._inner.matches
                    .reduce((res, obj) => {
                        ...
                        if (obj.otherwise) {
                            res.push(obj.otherwise);
                        }

                        return res;
                    }, [])
                    .map(joiNewObj => {
                        const altName = Utilities.getJoiLabel(joiNewObj) || name;
     ...
     * @return {Object}
     */
    public getDefinitionCollection(isAlt) {
     ...
     */
    public convertRules(property, rules, ruleNames, groupName) {
     ...
     */
    public appendToPropertyObject(property, rules, groupName, ruleName) {
     ...
     * @param  {String} value
     */
    public addToPropertyObject(property, groupName, ruleName, value) {
     ...
     */
    public getArgByName(array, name) {
     ...
     * @return {Boolean}
     */
    public hasPropertyByName(array, name) {
        return (
            array &&
            array.some(obj => {
