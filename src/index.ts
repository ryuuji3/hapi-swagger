            ...
            server.dependency(["inert", "vision"], server => {
                ...
                if (settings.debug === true) {
                    server.route([
                        {
                            method: "GET",
                            path: Path.join(settings.documentationPath, Path.sep, "debug")
                                .split(Path.sep)
                                .join("/"),
                            options: {
                                auth: settings.auth,
...
function appendDataContext(plugin, settings) {
    plugin.ext("onPostHandler", (request, h) => {
        ...
        if (response.variety === "view") {
            ...
            const uri = new URL(settings.jsonPath, "uri://"); // validate URL

            // append tags from document request to JSON request
            if (request.query.tags) {
                uri.searchParams.set("tags", request.query.tags);
                settings.jsonPath = `${uri.pathname}${uri.search}`;
            }
